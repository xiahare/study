...
public class KuduSample {
	...
	private static final String KUDU_MASTERS = System.getProperty("kuduMasters", "192.168.12.11:7051");
	...
	static void createExampleTable(KuduClient client, String tableName) throws KuduException {
		...
		// Set up the partition schema, which distributes rows to different tablets by
		// hash.
		// Kudu also supports partitioning by key range. Hash and range partitioning can
		// be combined.
	...
	static void scanTableAndCheckResults(KuduClient client, String tableName, int numRows) throws KuduException {
		...
		// Scan with a predicate on the 'key' column, returning the 'value' and "added"
		// columns.
		...
		KuduPredicate lowerPred = KuduPredicate.newComparisonPredicate(schema.getColumn("key"),
				ComparisonOp.GREATER_EQUAL, lowerBound);
		int upperBound = numRows / 2;
		KuduPredicate upperPred = KuduPredicate.newComparisonPredicate(schema.getColumn("key"), ComparisonOp.LESS,
				upperBound);
		...
		// Note: scanning a hash-partitioned table will not return results in primary
		// key order.
		int resultCount = 0;
		int nullCount = 0;
	...
	public static void stasticTable(KuduClient client,String tableName) throws KuduException {
		stasticTable(client, tableName, true);
	}
	public static void stasticTable(KuduClient client,String tableName, boolean displayNull) throws KuduException {
		KuduTableStatistics tableStistics = client.getTableStatistics(tableName);
		long rowCount = tableStistics.getLiveRowCount();
		long diskSize = tableStistics.getOnDiskSize();
		if(rowCount!=0) {
			
			System.out.println("Table:"+tableName + " | RowCount:"+rowCount + " | DiskSize:"+diskSize);
		}
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			try {
				...
				List<String> tables = client.getTablesList("java_example").getTablesList();
				...
				List<String> logtables = client.getTablesList().getTablesList();
				for(String table:logtables) {
					stasticTable(client, table, false);
				}
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
